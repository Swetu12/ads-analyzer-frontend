...
const Hero: React.FC = () => {
  ...
  const scrollYTransformer = useTransform(
    scrollYProgress,
    [0, 1],
    [0.85, 1.08],
  );

  const scale = useSpring(scrollYTransformer, {
    stiffness: 300,
    damping: 30,
    restDelta: 0.001,
  });
  ...
  return (
    <section className={"py-10 md:py-16"}>
      ...
      >
        ...
        <div
          className={
            "relative mt-12 max-w-screen-xl mx-auto isolate rounded-xl md:mt-16"
          }
        >
          <motion.figure
            ref={heroBannerRef}
            ...
            className={
              "bg-background/60 border border-slate-800 backdrop-blur-3xl rounded-xl shadow-2xl overflow-hidden"
            }
          >
            <img
              src={`/hero-banner.png`}
          ...
          <motion.div
            ...
            className={
              "absolute inset-0 bg-primary blur-[200px] scale-y-75 scale-x-125 rounded-full -z-10"
            }
          ></motion.div>
        </div>
      </motion.div>
